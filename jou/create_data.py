...
if __name__ == "__main__":
    ...
    numScen = np.ones(HORIZON - 1, np.int32) * 5            #5 scenarios for stages 2,...,end
    numScen = np.insert(numScen, 0, 1)                      #1 scenario for stage 1
    ...
    D0 = 0.57                                                               #starting demand
    scenarios = [[0] * numScen[i] for i in range(HORIZON)]                 #scenarios for each stage in inner list
    scenarios[0][0] = [0] * 2 * SUBPERIOD                                   # 2 used because one is for gas price and other for demand                             
    
    #not generation for scenario 0
    for t in range(1, HORIZON):
        sample = D0 * (1.01 ** t - 1.005 ** t) * np.random.sample((5,)) + D0 * 1.005 ** t
        scenarios[t] = list(sample)

        for j in range(numScen[t]):
            print("sub: ", max(scenarios[t][j] - D0, 0.0))
            print("before: ", LAMBDA * max(scenarios[t][j] - D0, 0.0) * 1e9 )
            temp = LAMBDA * max(scenarios[t][j] - D0, 0.0) * 1e9 / HOURS_PER_YEAR
            temp = np.concatenate((temp, -temp))                                    #why do we have temp and -temp values here
            scenarios[t][j] = list(temp)                                            
            print(scenarios[t][j])                                                  #seems to be telling the demand in three different scenarios                                                     
    ...
    myFile.write("[")
    for i in range(valueLB.shape[0]):
    ...
    xCoef = np.zeros((HORIZON, 2 * (nUnit + nType)))                                       #why is xCoeff taken like this?
    ...
    myFile.write("[")
    for i in range(uncertainIndex.shape[0]):
