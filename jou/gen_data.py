...
if __name__ == "__main__":
	...
	demand_tree = [[0] * numScen[i] for i in range(HORIZON)]
	demand_tree[0][0] = [0.0] * SUBPERIOD

	for t in range(1, HORIZON):
		r_v = np.random.sample((scenPerStage,))
		print(r_v)
		sample = D0 * (1.015 ** t - 1.005 ** t) * r_v + D0 * 1.005 ** t
	...
	price = [[0] * numScen[t] for t in range(HORIZON)]
	...
	myFile.write("[")
	for i in range(MAX_OUTPUT.shape[0]):
	...
	myFile.write("[")
	for i in range(MAX_UNIT.shape[0]):
	...
	yCoef = []
	for t in np.arange(HORIZON):
		yCoef_t = [ [0] * SUBPERIOD for n in range(numScen[t]) ]
		for n in range(numScen[t]):
	...
	myFile.write("[")
	for t in range(zCoef.shape[0]):
		myFile.write("[")
		for k in range(zCoef.shape[1]):
	...
	# y2Coef scenarios
	y2Scenarios = [ [0] * numScen[t] for t in range(HORIZON) ]
	...
	myFile.write("[")
	for row in range(y1Coef.shape[0]):
		myFile.write("[")
		for column in range(y1Coef.shape[1]):
	...
	myFile.write("[")
	for row in range(y2Coef.shape[0]):
		myFile.write("[")
		for column in range(y2Coef.shape[1]):
	...
	for t in np.arange(HORIZON):
		...
		for row in range(W2.shape[0]):
			myFile.write("[")
			for column in range(W2.shape[1]):
	...
	myFile.write("[")
	for i in range(uncertainIndexRHS.shape[0]):
